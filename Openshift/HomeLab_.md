...
|---|---|---|---|---|---|---|---|---|
|services.ocp-cluster.okd.local|	192.168.22.254|	4	|16	|150	|haproxy, dns, registry, nfs, dhcp|	centos8|	08:00:27:EF:FA:01|08:00:27:EF:FA:02
|bootstrap.okd-lab.okd.lab|	192.168.22.2|	4|	16|	100	|bootstrap| CoreOS|	08:00:27:05:13:82|
|master0.okd-lab.okd.lab|	192.168.22.3|	4|	16|	150|	master|	CoreOS|	08:00:27:05:13:83|
|master1.okd-lab.okd.lab|	192.168.22.4|	4|	16|	100|	master|	CoreOS|	08:00:27:05:13:84|
|master2.okd-lab.okd.lab|	192.168.22.5|	4|	16|	100|	master|	CoreOS|	08:00:27:05:13:85|
|worker0.okd-lab.okd.lab|	192.168.22.6|	2|	8	|100|	worker|	CoreOS|	08:00:27:05:13:86|
|worker1.okd-lab.okd.lab|	192.168.22.7|	2|	8	|100|	worker|	CoreOS|	08:00:27:05:13:87|
...
```shell
# Set hostname
$ hostnamectl set-hostname services.ocp-cluster.rdss.cloud
```
...
subnet 192.168.22.0 netmask 255.255.255.0 {
  range 192.168.22.2 192.168.22.253;
  option domain-name-servers 192.168.22.254; # Option 6
  option ntp-servers 192.168.22.254;  # Option 42
  option routers 192.168.22.254; # Replace with your actual gateway if different
}

host bootstrap.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:82;
  fixed-address 192.168.22.2; # Replace with the desired IP address for this MAC
}

host master1.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:83;
  fixed-address 192.168.22.3; # Replace with the desired IP address for this MAC
}

host master2.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:84;
  fixed-address 192.168.22.4; # Replace with the desired IP address for this MAC
}

host master3.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:85;
  fixed-address 192.168.22.5; # Replace with the desired IP address for this MAC
}

host worker0.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:86;
  fixed-address 192.168.22.6; # Replace with the desired IP address for this MAC
}

host worker1.ocp-cluster.rdss.cloud {
  hardware ethernet 08:00:27:05:13:87;
  fixed-address 192.168.22.7; # Replace with the desired IP address for this MAC
}
EOL
```
...
sed -i 's/listen-on port 53 { 127.0.0.1; };/listen-on port 53 { 127.0.0.1; 192.168.22.254; };/g' /etc/named.conf
sed -i 's/allow-query     { localhost; };/allow-query     { localhost; 192.168.22.0\/24; };/g' /etc/named.conf
...
zone "ocp-cluster.rdss.cloud" {
    type master;
    file "/etc/named/zones/db.ocp-cluster.rdss.cloud"; # zone file path
};

zone "22.168.192.in-addr.arpa" {
    type master;
    file "/etc/named/zones/db.192.168.22";  # 192.168.22.0/24 subnet
};
EOL
```
...
# Forward Zone
cat <<EOL > /etc/named/zones/db.ocp-cluster.rdss.cloud
\$TTL 1W
@ IN SOA services.ocp-cluster.rdss.cloud. root (
   2019070700 ; serial
   3H  ; refresh (3 hours)
   30M  ; retry (30 minutes)
   2W  ; expiry (2 weeks)
   1W )  ; minimum (1 week)
 IN NS services.ocp-cluster.rdss.cloud.
;
;
services.ocp-cluster.rdss.cloud.  IN A 192.168.22.254
;
helper.ocp-cluster.rdss.cloud.  IN A 192.168.22.254
helper.ocp-cluster.rdss.cloud. IN A 192.168.22.254
;
api.ocp-cluster.rdss.cloud.  IN A 192.168.22.254
api-int.ocp-cluster.rdss.cloud. IN A 192.168.22.254 
;
*.apps.ocp-cluster.rdss.cloud. IN A 192.168.22.254 
;
bootstrap.ocp-cluster.rdss.cloud. IN A 192.168.22.2 
;
master0.ocp-cluster.rdss.cloud. IN A 192.168.22.3 
master1.ocp-cluster.rdss.cloud. IN A 192.168.22.4 
master2.ocp-cluster.rdss.cloud. IN A 192.168.22.5 
;
worker0.ocp-cluster.rdss.cloud. IN A 192.168.22.6 
worker1.ocp-cluster.rdss.cloud. IN A 192.168.22.7 
;
EOL

# Reverse Zone
cat <<EOL > /etc/named/zones/db.192.168.22
\$TTL 1W
@ IN SOA services.ocp-cluster.rdss.cloud. root (
   2019070700 ; serial
   3H  ; refresh (3 hours)
...
2.22.168.192.in-addr.arpa. IN PTR bootstrap.ocp-cluster.rdss.cloud. 
;
3.22.168.192.in-addr.arpa. IN PTR master0.ocp-cluster.rdss.cloud. 
4.22.168.192.in-addr.arpa. IN PTR master1.ocp-cluster.rdss.cloud. 
5.22.168.192.in-addr.arpa. IN PTR master2.ocp-cluster.rdss.cloud. 
;
6.22.168.192.in-addr.arpa. IN PTR worker0.ocp-cluster.rdss.cloud. 
7.22.168.192.in-addr.arpa. IN PTR worker1.ocp-cluster.rdss.cloud. 
;
254.22.168.192.in-addr.arpa. IN PTR api.ocp-cluster.rdss.cloud. 
254.22.168.192.in-addr.arpa. IN PTR api-int.ocp-cluster.rdss.cloud. 
;
EOL
```
...
# Check forward zone
$ sudo named-checkzone ocp-cluster.rdss.cloud /etc/named/zones/db.ocp-cluster.rdss.cloud

# Check Reverse Zone
$ named-checkzone 22.168.192.in-addr.arpa /etc/named/zones/db.192.168.22
```
...
$ dig +noall +answer @192.168.22.254 api.ocp-cluster.rdss.cloud
$ dig +noall +answer @192.168.22.254 api-int.ocp-cluster.rdss.cloud
$ dig +noall +answer @192.168.22.254 random.apps.ocp-cluster.rdss.cloud
$ dig +noall +answer @192.168.22.254 console-openshift-console.apps.ocp-cluster.rdss.cloud
$ dig +noall +answer @192.168.22.254 bootstrap.apps.ocp-cluster.rdss.cloud

api.ocp-cluster.rdss.cloud.          604800  IN      A       192.168.22.254
api-int.ocp-cluster.rdss.cloud.      604800  IN      A       192.168.22.254
random.apps.ocp-cluster.rdss.cloud.  604800  IN      A       192.168.22.254
console-openshift-console.apps.ocp-cluster.rdss.cloud. 604800 IN A 192.168.22.254
bootstrap.apps.ocp-cluster.rdss.cloud. 604800 IN     A       192.168.22.254

# Reverse DNS Validation

$ dig +noall +answer @192.168.22.254 -x 192.168.22.254
$ dig +noall +answer @192.168.22.254 -x 192.168.22.2
$ dig +noall +answer @192.168.22.254 -x 192.168.22.3
$ dig +noall +answer @192.168.22.254 -x 192.168.22.4
$ dig +noall +answer @192.168.22.254 -x 192.168.22.5
$ dig +noall +answer @192.168.22.254 -x 192.168.22.6
$ dig +noall +answer @192.168.22.254 -x 192.168.22.7

2.22.168.192.in-addr.arpa. 604800 IN    PTR     bootstrap.ocp-cluster.rdss.cloud.
3.22.168.192.in-addr.arpa. 604800 IN    PTR     master0.ocp-cluster.rdss.cloud.
4.22.168.192.in-addr.arpa. 604800 IN    PTR     master1.ocp-cluster.rdss.cloud.
5.22.168.192.in-addr.arpa. 604800 IN    PTR     master2.ocp-cluster.rdss.cloud.
6.22.168.192.in-addr.arpa. 604800 IN    PTR     worker0.ocp-cluster.rdss.cloud.
7.22.168.192.in-addr.arpa. 604800 IN    PTR     worker1.ocp-cluster.rdss.cloud.
```
...
-addext "subjectAltName = DNS:services.ocp-cluster.rdss.cloud"

$ cp /opt/registry/certs/domain.crt /etc/pki/ca-trust/source/anchors/
$ update-ca-trust

$ trust list | grep -i "services.ocp-cluster.rdss.cloud"
...
$ curl https://services.ocp-cluster.rdss.cloud:5000/v2/_catalog

# Login
$ podman login services.ocp-cluster.rdss.cloud:5000
...
listen api-server-6443 
  ...
  balance roundrobin
  server bootstrap bootstrap.ocp-cluster.rdss.cloud:6443 verify none check check-ssl inter 10s fall 2 rise 3 backup 
  server master0 master0.ocp-cluster.rdss.cloud:6443 weight 1 verify none check check-ssl inter 10s fall 2 rise 3
  server master1 master1.ocp-cluster.rdss.cloud:6443 weight 1 verify none check check-ssl inter 10s fall 2 rise 3
  server master2 master2.ocp-cluster.rdss.cloud:6443 weight 1 verify none check check-ssl inter 10s fall 2 rise 3
listen machine-config-server-22623 
  bind *:22623
  mode tcp
  server bootstrap bootstrap.ocp-cluster.rdss.cloud:22623 check inter 1s backup 
  server master0 master0.ocp-cluster.rdss.cloud:22623 check inter 1s
  server master1 master1.ocp-cluster.rdss.cloud:22623 check inter 1s
  server master2 master2.ocp-cluster.rdss.cloud:22623 check inter 1s
listen ingress-router-443 
  bind *:443
  mode tcp
  balance source
  server worker0 worker0.ocp-cluster.rdss.cloud:443 check inter 1s
  server worker1 worker1.ocp-cluster.rdss.cloud:443 check inter 1s
listen ingress-router-80 
  bind *:80
  mode tcp
  balance source
  server worker0 worker0.ocp-cluster.rdss.cloud:80 check inter 1s
  server worker1 worker1.ocp-cluster.rdss.cloud:80 check inter 1s
EOL
...
acl internalnet src 192.168.22.0/24
http_access allow internalnet
http_access deny all

# If the services host does not access Internet directly and requires a proxy,
...
$ cat <<EOL > /etc/exports
/mnt/nfs_shares/registry  192.168.22.0/24(rw,sync,no_all_squash,root_squash)
EOL
...
platform:
  none: {}
proxy:
  httpProxy: http://services.ocp-cluster.rdss.cloud:3128
  httpsProxy: http://services.ocp-cluster.rdss.cloud:3128
  noProxy: rdss.cloud,192.168.22.0/24
publish: External
...
sshKey: ssh-rsa  root@services.ocp-cluster.rdss.cloud
EOL
```
...
INFO Access the OpenShift web-console here: https://console-openshift-console.apps.ocp-cluster.rdss.cloud
